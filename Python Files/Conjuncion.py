...
def determineCheapestPathDijkstra(mapas, dictionary, caminos, pisosE):
    ...
    print("Estos son los pisos empleados: ", pisosEmpleados)
...
def main():
    ...
    if salidaPredeterminada:
        ...
        beaconName = 'CDTC 308'
        pisoE = data[beaconName][0]['piso']
        #print("Este es el piso de inicio", type(pisoE))

        if not beaconName in dictBeaconName_ID:
            print("Beacon de partida no disponible o inexistente.")
            sys.exit(1)
        else:
            executeAlgorithm(g, beacons, escaleras, salidas, mapas, beaconName, beaconsDesactivados, dictBeaconName_ID, pisoE)
